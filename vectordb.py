...
### 4️⃣ PROCESS PDF FILE FOR STORAGE ###
...
def store_pdf_in_chroma(uploaded_file, vectordb):
    """
    Stores the processed document chunks into ChromaDB.

    Args:
        uploaded_file: File for RAG ingestion pipeline.
        vectordb: Instance of Chroma vector store.
...
### 6️⃣ RETRIEVE DOCUMENTS FROM CHROMADB ###
def retrieve_from_chroma(query, vectordb, top_k=5):
    """
...
### 7️⃣ RETRIEVE RELEVANT TEXT FROM CHROMADB ###
def retrieve_relevant_text(query, vectordb, top_k=5):
    """
